...
# Paths
save_dir = '../results/'
save_pt_best = f'Best_PINN_E{epochs}.pt'
save_pt = f'PINN_E{epochs}.pt'
save_txt = f'PINN_E{epochs}.yml'

checkpoint_path= f'checkpoint_PINN_E{epochs}.pt'
...
class UNetModel(BaseModel):
    ...
    def forward_loss(self, data):
        ...
        pinn = torch.cat((input,output),axis=1)[:,None,-3:,:,:]

        loss = self.loss_fn(output, label, pinn)

        return loss[0], {'Loss':loss[0], 'Loss PINN':loss[1]} #,'Loss MSE':loss[1],'Loss GDL':loss[2],'Loss MAE':loss[3]} # Elements in the dict : only for printing
