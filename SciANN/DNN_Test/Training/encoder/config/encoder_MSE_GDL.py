...
# Paths
save_dir = '../results/'
save_pt_best = f'Best_L2_GDL_E{epochs}.pt'
save_pt = f'L2_GDL_E{epochs}.pt'
save_txt = f'L2_GDL_E{epochs}.yml'

checkpoint_path= f'checkpoint_L2_GDL_E{epochs}.pt'
...
class AutoEncoderModel(BaseModel):
    def __init__(self, net, opt=None, sched=None, logger=None, print_progress=True, device='cuda:0'):
        ...
        self.loss_fn = MSLoss(*[nn.MSELoss(),GDLLoss()]) #*[GDLLoss(),nn.MSELoss(),nn.L1Loss()])
    ...
    def forward_loss(self, data):
        ...
        return loss[0], {'Loss':loss[0], 'Loss MSE':loss[1], 'Loss GDL':loss[2]} # Elements in the dict : only for printing

# Create the model
model = AutoEncoderModel(net, opt=optimizer, sched=None, logger=None, print_progress=True, device=device)
